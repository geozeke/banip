...
def archive_changelog(
    ...
    for major_minor, moved in inactive.items():
        ...
        archive_preamble = (
            ...
            f"Archived banip releases for the {major_minor[0]}.{major_minor[1]}.x "
            "minor version line."
        )
...
def project_versions(project_root: Path) -> dict[str, str]:
    ...
    locked_project = next(
        (package for package in uv_lock["package"] if package["name"] == "banip"),
        None,
    )
    if locked_project is None:
        raise ValueError("banip is missing from uv.lock")
    return {
...
def validate_changelog_collection(
    ...
    for path in paths:
        ...
        if len(second_level_headings) != len(sections):
        ...
        for section, version in zip(sections, versions, strict=True):
            ...
            if heading is None or not heading.group("date"):
            ...
            if path != changelog_path:
                expected_name = f"v{version.major}.{version.minor}.x.md"
                if path.name != expected_name:
                    raise ValueError(
                        f"{section.label} belongs in {expected_name}, not {path.name}"
                    )
            groups = [
                group.group("group")
