...
def split_changelog(text: str) -> tuple[str, list[ReleaseSection], dict[str, str]]:
    ...
    tuple[str, list[ReleaseSection], dict[str, str]]
        Preamble text, release sections, and link reference definitions keyed
        by label.
    """
...
def archive_changelog(
    ...
    ----------
    version
        Target semantic version, with or without a leading ``v`` and optional
        prerelease or build metadata.
    changelog_path
    ...
    force
        Archive any non-target minor line even when the target appears to be a
        patch update.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Archive old minor-version changelog entries.",
    )
    ...
    parser.add_argument(
        ...
        help="Path to the active changelog file.",
    )
    parser.add_argument(
        "--archive-dir",
        type=Path,
        default=Path("changelogs"),
        help="Directory for archived changelog files.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Archive non-target minor lines even for patch updates.",
    )
