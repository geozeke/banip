...
def test_compact_can_disable_or_create_subnets() -> None:
    ...
    loose_ips, loose_nets = utilities.compact(ips, allowlist=[], min_num=0)
    compact_ips, compact_nets = utilities.compact(ips, allowlist=[], min_num=2)
...
def test_compact_respects_allowlisted_networks() -> None:
    """Allowlisted ranges prevent subnet compaction."""
    ips = [ipa.ip_address("192.0.2.1"), ipa.ip_address("192.0.2.3")]
    allowlist = [ipa.ip_network("192.0.2.0/28")]

    compact_ips, compact_nets = utilities.compact(ips, allowlist=allowlist, min_num=2)
...
def test_load_rendered_blocklist_splits_file(tmp_path, monkeypatch) -> None:
    """Rendered blocklist data is loaded as sorted IP and network lists."""
    ...
    ips, nets = utilities.load_rendered_blocklist()
