...
Country reference for two-letter codes used in `banip.yaml` targets.
...
| Colombia 🇨🇴 `CO` | Comoros 🇰🇲 `KM` | Congo Republic 🇨🇬 `CG` |
...
| French Southern Territories 🇹🇫 `TF` | Gabon 🇬🇦 `GA` | Georgia 🇬🇪 `GE` |
...
| Saint Helena 🇸🇭 `SH` | Saint Lucia 🇱🇨 `LC` | Saint Martin 🇲🇫 `MF` |
| Saint Pierre and Miquelon 🇵🇲 `PM` | Samoa 🇼🇸 `WS` | San Marino 🇸🇲 `SM` |
...
| Spain 🇪🇸 `ES` | Sri Lanka 🇱🇰 `LK` | St Kitts and Nevis 🇰🇳 `KN` |
| St Vincent and Grenadines 🇻🇨 `VC` | Sudan 🇸🇩 `SD` | Suriname 🇸🇷 `SR` |
| Svalbard and Jan Mayen 🇸🇯 `SJ` | Sweden 🇸🇪 `SE` | Switzerland 🇨🇭 `CH` |
| Syria 🇸🇾 `SY` | São Tomé and Príncipe 🇸🇹 `ST` | Taiwan 🇹🇼 `TW` |
| Tajikistan 🇹🇯 `TJ` | Tanzania 🇹🇿 `TZ` | Thailand 🇹🇭 `TH` |
| The Gambia 🇬🇲 `GM` | The Netherlands 🇳🇱 `NL` | Timor-Leste 🇹🇱 `TL` |
| Togo 🇹🇬 `TG` | Tokelau 🇹🇰 `TK` | Tonga 🇹🇴 `TO` |
| Trinidad and Tobago 🇹🇹 `TT` | Tunisia 🇹🇳 `TN` | Turkmenistan 🇹🇲 `TM` |
| Turks and Caicos Islands 🇹🇨 `TC` | Tuvalu 🇹🇻 `TV` | Türkiye 🇹🇷 `TR` |
| U.S. Outlying Islands 🇺🇲 `UM` | U.S. Virgin Islands 🇻🇮 `VI` | Uganda 🇺🇬 `UG` |
